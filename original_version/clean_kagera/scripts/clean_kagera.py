...
cleaned_sample_sheet=snakemake.output.cleaned_sample_sheet
...
def fastq_sample_sheet(fastq_dict, original_fastq, cleaned_fastq_script, cleaned_sample_sheet):
	'''
	...
	copy fastq files of interest. Also outputs sample sheets that can be used
	for wrangling the data.
	'''
	flipped_dict={fastq_dict[key]:key for key in fastq_dict}
	cleaned_fastq_script=open(cleaned_fastq_script, 'w')
	cleaned_sample_sheet=open(cleaned_sample_sheet, 'w')
	cleaned_sample_sheet.write('sample_name\tsample_set\treplicate\tprobe_set\tlibrary_prep\n')
	count=0
	...
	for line in open(original_fastq):
		...
		if sample_lookup in flipped_dict:
			sample_set=year+'-MSMT'
			replicate='1'
			probe_set='DR23K'
			library_prep='placeholder'
			to_replace='-'+sample_set+'-'+replicate
			sample_name=flipped_dict[sample_lookup].replace(to_replace, '')
			if line.endswith('_R1_001.fastq.gz'):
				cleaned_sample_sheet.write('\t'.join([sample_name, sample_set, replicate, probe_set, library_prep])+'\n')
...
fastq_sample_sheet(fastq_dict, original_fastq, cleaned_fastq_script, cleaned_sample_sheet)