...
'present' within a sample (if coverage and alternate UMI counts meet or exceed
the respective threshold numbers)', 'covered' (if coverage UMI count meets the
...
folder include:
 ...
 - within_sample_allele_frequencies.csv: these are the number of UMIs that
   contain the mutation within a sample divided by the number of total UMIs
   within the sample that 'cover' the genomic region.
